-- todo:
-- 1. Note: if a field is recognized but the relevant feature is disabled, it's not an "unknown field".
-- 2. Sequence number overflow
-- 3. Colorize "unknown field"
-- 4. for a field that appears out-of-order, display "out-of-order field " in red
...
-- NDN protocol
ndn = Proto("ndnlpv2", "Named Data Networking Link Protocol(NDNLP Packet)")
...
function getDetail(b)
   local identifier = getNonNegativeInteger(b)
   if(identifier == 0) then return "None"
   elseif(identifier == 50) then return "Congestion"
   elseif(identifier == 100) then return "Duplicate"
   elseif(identifier == 150) then return "NoRoute"
   elseif(identifier == 1) then return "NoCache"
   else return nil
   end
end
...
local AppPrivateBlock3 = 960
function canIgnore(t)
   if (t < AppPrivateBlock2 or t >= AppPrivateBlock3) then
      return false
   else
      local mod = math.fmod( t, 2 ) 
      if (mod == 1) then return true
      else return false
      end
   end
   return false
...
local NDN_DICT = {
   ...
   [801] = {name = "NackReason"			 , field = ProtoField.string("ndn.nack_reason", "NackReason"), value = getDetail},
   ...
   [821] = {name = "CachePolicyType"		 , field = ProtoField.string("ndn.cachepolicy_type", "CachePolicyType"), value = getDetail},

  -- Unknown Packet field
   [803] = {name = "unknown field (ignored)"},
   [805] = {name = "unknown field"}
}
...
function canBeValidNdnPacket(block)
   if ((block.type == 100) and block.length <= 8800) then
      return true
   else
      return false
   end
...
-- websocketDissectorTable:add("9690", ndn)
